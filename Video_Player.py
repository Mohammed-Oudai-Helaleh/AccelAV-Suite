...
class VideoPlayer:
    ...
    def __init__(self, canvas, control_frame):
        ...
        self.full_audio_array = None  # Keep original audio array
    ...
    def load_video(self, video_path):
        ...
        if video_path.lower().endswith(('.mp3', '.wav', '.m4a', '.ogg', '.flac')):
            ...
            try:
                if video_path.lower().endswith('.wav'):
                    ...
                    if self.audio_sample_rate != 44100:
                        if audio_data.ndim > 1:
                            ...
                            for ch in range(audio_data.shape[1]):
                                # Convert channel data to float in [-1, 1]
                                ...
                                # Convert back to int16 without extra scaling
                                resampled_channels.append(channel_resampled.astype(np.int16))
                            # Stack channels back into a 2D array (columns = channels)
                    ...
                    # Do not average channels—preserve stereo data.
                    self.audio_array_int = audio_data
                    
                    # For visualization, use the first channel (if stereo) or the mono data.
                    self.audio_data_normalized = (audio_data[:, 0] if audio_data.ndim > 1 else audio_data).astype(np.float32) / 32767.0

                else:
                    # For non-WAV files, load as mono with librosa and convert to int16.
                    ...
                    # For visualization, keep the mono data.
                    self.audio_data_normalized = self.audio_array_int.astype(np.float32) / 32767.0

                # Ensure playback data is stereo. If mono, duplicate the channel.
                ...
                # Create pygame sound and compute total playback time.
        ...
        try:
            with VideoFileClip(self.video_path) as video_clip:
                ...
                self.audio_sample_rate = 44100
                self.audio_array_int = audio_array_int
                self.audio_sound = pygame.sndarray.make_sound(self.audio_array_int)
                if audio_clip.reader:
